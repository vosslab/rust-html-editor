...
pub fn setup_menu(app: &tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    // -- Rust HTML Editor menu --
    let rust_html_editor_about = PredefinedMenuItem::about(app, Some("About Rust HTML Editor"), None)?;
    let rust_html_editor_hide = PredefinedMenuItem::hide(app, None)?;
    let rust_html_editor_hide_others = PredefinedMenuItem::hide_others(app, None)?;
    let rust_html_editor_show_all = PredefinedMenuItem::show_all(app, None)?;
    let rust_html_editor_quit = PredefinedMenuItem::quit(app, None)?;

    let rust_html_editor_menu = SubmenuBuilder::new(app, "Rust HTML Editor")
        .item(&rust_html_editor_about)
        .separator()
        .item(&rust_html_editor_hide)
        .item(&rust_html_editor_hide_others)
        .item(&rust_html_editor_show_all)
        .separator()
        .item(&rust_html_editor_quit)
        .build()?;
    ...
    let open_file = MenuItemBuilder::with_id("open_file", "Open File...")
        ...
        .build(app)?;
    let open_project = MenuItemBuilder::with_id("open_project", "Open Folder...")
    ...
    let export = MenuItemBuilder::with_id("export", "Export to Browser")
        .build(app)?;
    let import_markdown = MenuItemBuilder::with_id("import_markdown", "Import Markdown...")
        .build(app)?;
